...
利用者が毎回すべてのコマンドを手動実行する必要はない\cite{latex2e-crossrefs,latexmk}．
...
Overleafはエディタとサーバ上のTeX Liveを一体として提供する\cite{overleaf-compiler}．
...
\textbf{どのプログラムを，どの順番で，どの設定によって実行するか}を共有することである\cite{latexmk,overleaf-latexmkrc}．
...
利用するテンプレートや投稿先の指定がある場合は，その方式を優先する\cite{overleaf-bibliographies}．
...
単純なフローチャート，ノードと矢印，幾何図形などは，画像編集ソフトを使わずに作成できる\cite{pgf-tikz-manual}．
